"""Fetch and parse iCal (.ics) URLs into the canonical events-nrw schema.

Used by tools/scrape_venue_events.py for venues whose `kind: ical`. May also be
imported standalone for one-off probes.

Canonical Event schema (dict, keys match projects/events-nrw/CLAUDE.md):
...
DEFAULT_HEADERS = {
    "User-Agent": (
        ...
        "events-nrw-calendar/0.1 (+private use, contact: fengelh@gmail.com)"
    ),
