...
Layout philosophy (see projects/events-nrw/CLAUDE.md and the agreed mockup):
...
# "Recently Added" strip: events stamped with first_seen within the last
# NEW_FRESH_DAYS get an inline NEW badge; up to NEW_STRIP_CAP land in the
...
def _event_identity(e) -> tuple[str, str, str]:
    ...
    rebuild_calendar._event_seen_key (minus title normalisation) — inside
    the renderer we trust the orchestrator's first_seen field and just need
    a deterministic hashable key for set membership."""
...
def render(
    ...
    # 3b. "Recently Added" — events with first_seen in the last NEW_FRESH_DAYS,
    # excluding anything already in "Don't Miss". Capped at NEW_STRIP_CAP;
    # inline NEW badges in the agenda below tag the same fresh keys.
    ...
    html_text = _render_html(
        title=title,
        subtitle=subtitle or _long_date(now),
        now=now,
        ...
        lang=lang,
        city_identity_word=city_identity_word,
...
def _collect_venues_by_city(
    ...
    targeting the same physical venue (e.g. `kunstpalast` + `kunstpalast-current`)
    collapse into ONE chip that controls both. chip_id is `<city>-<name-slug>`.
    ...
    # Suppress chips with <2 events UNLESS the chip controls a venue_id
    # explicitly configured in venues.yaml (appears in venue_meta). This
    # keeps real single-event venues — static/permanent exhibitions, venues
    # in transition — visible while still filtering phantom one-off venues
    # spawned by aggregators.
...
def _week_label(key: tuple[int, int], this_year: int, this_week: int, first_event) -> str:
    ...
    # US-style date span: "May 4–10" (single month) or "May 28 – Jun 3" (cross-month)
    if monday.month == sunday.month:
        span = f"{MONTHS[monday.month - 1]} {monday.day}–{sunday.day}"
    else:
        span = f"{MONTHS_SHORT[monday.month - 1]} {monday.day} – {MONTHS_SHORT[sunday.month - 1]} {sunday.day}"
...
def _long_date(dt: datetime) -> str:
    """US-style long date: 'May 9, 2026'."""
    return f"{MONTHS[dt.month - 1]} {dt.day}, {dt.year}"


def _us_time(dt: datetime) -> str:
    """US 12-hour time without leading zero: '7:30 PM'. Cross-platform."""
...
def _render_html(
    ...
    _CITIES = [("hk","🇨🇳","Hong Kong"),("hk-kids","👶","HK Kids"),
               ("la","🇺🇸","Los Angeles"),("nrw","🇩🇪","NRW"),
               ("singa","🇸🇬","Singapore"),("fukuoka","🇯🇵","Fukuoka")]
    ...
    for code, flag, label in _CITIES:
        active = ' is-active' if code == city_code else ''
        parts.append(f'    <a class="city-switcher__chip{active}" href="../{code}/">{flag} {html.escape(label)}</a>')
    parts.append('  </div>')
    parts.append('</nav>')
    ...
    if header_eyebrow:
        # Tint the city-identity word ("Singa") so it reads as a logo
        # accent. HTML is composed (not escaped) so the <span> survives;
        # eyebrow text comes from trusted config.
        eb_escaped = html.escape(header_eyebrow)
        eb_html = re.sub(
            r"(" + re.escape(city_identity_word) + r")",
    ...
    parts.append('    <input type="search" id="search-box" class="search-input" '
                 'placeholder="Search — e.g. Beethoven, LACMA, ballet…" '
    ...
    parts.append('  <nav class="filter-bar filter-bar-city" aria-label="Filter by neighborhood">')
    parts.append('    <span class="filter-label">Where</span>')
    parts.append('    <label for="c-all" class="filter-chip filter-all">All</label>')
    ...
    parts.append('  </nav>')
    parts.append('  <nav class="filter-bar filter-bar-when" aria-label="Filter by time">')
    parts.append('    <span class="filter-label">When</span>')
    parts.append('    <label for="w-all" class="filter-chip filter-all">Anytime</label>')
    ...
    if new_events:
        ...
        if new_overflow > 0:
            parts.append(
                ...
                f'+ {new_overflow} more new events tagged below</p>'
            )
        parts.append('  </section>')
    ...
    if not week_groups:
        parts.append('    <p class="empty">No events found.</p>')
    ...
    # Footer
    parts.append('  <footer>')
    parts.append(f'    <p>Updated {html.escape(_long_date(now))} · {now.strftime("%H:%M")}</p>')
    parts.append('  </footer>')
    ...
    parts.append('  }')
    parts.append('})();')
    # GoatCounter event-click tracking: which events do users open?
    parts.append('(function(){')
    parts.append('  document.addEventListener("click",function(e){')
    parts.append('    var a=e.target.closest("a.row, a.featured-card"); if(!a) return;')
    parts.append('    if(!window.goatcounter||!window.goatcounter.count) return;')
...
def _render_featured_card(ev, now: datetime, fresh_keys: Optional[set] = None) -> str:
    ...
    if e is not None and e > now:
        ...
        date_line = f"through {MONTHS[e.month - 1]} {e.day} · {days_left} day{'s' if days_left != 1 else ''} left"
    else:
        date_line = (
            f'{WEEKDAYS[s.weekday()]}, {MONTHS[s.month - 1]} {s.day}'
            f' · {_us_time(s)}'
        )
    ...
    if extras:
        ...
        date_line += f" · + {len(extras)} more {'date' if len(extras) == 1 else 'dates'} through {last_str}"
...
def _render_ongoing_block(evs: list, now: datetime, featured: set,
                            fresh_keys: Optional[set] = None) -> str:
    """Render ongoing exhibitions under a 'Now Showing' heading at the top
...
def _render_row(ev, now: datetime, featured: set, fresh_keys: Optional[set] = None) -> str:
    ...
    if e is not None and e > now and s <= now:
        time_display = f"thru {MONTHS_SHORT[e.month - 1]} {e.day}"
    ...
    return f"""        <a class="{' '.join(classes)}" href="{url}" target="_blank" rel="noopener noreferrer" data-venue="{venue_id_attr}" data-when="{when_attr}">
          ...
          {pill_html}
          <button type="button" class="fav-btn" aria-label="Mark as favorite">♡</button>
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* "Recently Added" strip — mirrors .featured but with a teal accent */
    ...
    /* City switcher — top nav linking the 4 sister calendars */
    .city-switcher {{
      display: flex; align-items: center; justify-content: space-between; gap: 12px;
      max-width: 1200px; margin: 0 auto; padding: 10px 16px;
      border-bottom: 1px solid var(--line, #e5e5e5); flex-wrap: wrap;
    }}
    .city-switcher__home {{
      color: var(--muted, #666); text-decoration: none; font-size: 13px;
    }}
    .city-switcher__home:hover {{ color: var(--accent, #0a8a72); }}
    .city-switcher__chips {{ display: flex; gap: 6px; flex-wrap: wrap; }}
    .city-switcher__chip {{
    ...
    .city-switcher__chip:hover {{ background: rgba(0,0,0,0.08); color: inherit; }}
    .city-switcher__chip.is-active {{
      background: #181818; color: #fff; font-weight: 500;
    }}
    @media (prefers-color-scheme: dark) {{
      .city-switcher__chip {{ background: rgba(255,255,255,0.06); }}
      .city-switcher__chip:hover {{ background: rgba(255,255,255,0.12); }}
      .city-switcher__chip.is-active {{ background: #f5f5f5; color: #181818; }}
    }}
  </style>
