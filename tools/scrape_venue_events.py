...
projects/events-nrw/CLAUDE.md. A single source row may produce events under
...
@dataclass
class Event:
    """Canonical event row. Matches the schema in projects/events-nrw/CLAUDE.md."""
...
def scrape(venue_row: dict, session: Optional[requests.Session] = None) -> list[Event]:
    ...
    workflow's responsibility (see projects/events-nrw/workflows/rebuild_calendar.md).
    """
    ...
    try:
        ...
        elif kind == "playwright_detail_pages":
...
def _scrape_playwright_detail_pages(venue_row: dict, session=None) -> list[Event]:
    """Hybrid: render listing in Playwright to discover detail URLs, then
    fetch each detail page via plain requests for title + date extraction.

    Use when the listing page is JS-rendered (so detail URLs aren't in the
    static HTML) but the detail pages themselves ARE server-rendered with
    title + date visible to a regular HTTP fetch — saves the cost of a
    Playwright session per detail page.

    Config (in addition to the playwright_html_list keys):
      selectors:
        item:  CSS selector on the listing page that matches each detail
               anchor (the @href is used as the detail URL).
        title: CSS selector evaluated on the DETAIL page.
        date:  CSS selector on the detail page.
      max_details: int — safety cap on detail fetches (default 30).
      detail_url_filter: optional regex; only URLs matching are kept.
    """
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
    ...
    listing_url = venue_row.get("calendar_url", "")
    sel = venue_row.get("selectors") or {}
    item_sel = sel.get("item")
    seed_urls = venue_row.get("detail_urls") or []   # optional hand-curated list
    if not item_sel and not seed_urls:
        log.warning("%s: playwright_detail_pages needs selectors.item OR detail_urls", venue_row["id"])
        return []
    ...
    timeout_ms = int(venue_row.get("timeout_ms", 45000))
    extra_wait_ms = int(venue_row.get("extra_wait_ms", 3000))
    max_details = int(venue_row.get("max_details", 30))
    url_filter = venue_row.get("detail_url_filter")
    use_stealth = bool(venue_row.get("use_stealth"))

    # Step 1: discover detail URLs.
    # If `detail_urls` is provided in config, use that list directly (skip
    # listing render — useful when the listing page is unscrapable but
    # detail pages aren't, e.g. Cloudflare-walled landings + permissive
    # detail URLs surfaceable via DuckDuckGo).
    ...
    if seed_urls:
        ...
        for h in seed_urls:
            ...
            if len(detail_urls) >= max_details:
                break
        log.debug("%s: %d seed URLs from config", venue_row["id"], len(detail_urls))
    else:
        try:
            with sync_playwright() as p:
                browser = p.chromium.launch(headless=True)
                ...
                hrefs = page.eval_on_selector_all(
                    item_sel,
                    "els => els.map(e => e.tagName === 'A' ? e.href : (e.querySelector('a') ? e.querySelector('a').href : null)).filter(Boolean)",
                )
                browser.close()
            seen = set()
            for h in hrefs:
                if url_filter and not re.search(url_filter, h):
                    continue
                if h in seen:
                    continue
                seen.add(h)
            ...
            log.warning("%s: playwright listing render failed: %s", venue_row["id"], exc)
            return []

    log.debug("%s: %d detail URLs (seeded or scraped)", venue_row["id"], len(detail_urls))

    # Step 2: fetch each detail page. By default use plain requests (cheap),
    # but `detail_via_playwright: true` reuses Playwright for the detail
    # rendering — required when detail pages are also JS-rendered (HK Palace
    # Museum, M+). Shares one browser across all details for speed.
    use_pw_detail = bool(venue_row.get("detail_via_playwright"))
    detail_wait = int(venue_row.get("detail_extra_wait_ms", 2500))
    detail_html_by_url: dict[str, str] = {}
    if use_pw_detail:
        # use_stealth + fresh context per URL: bypasses Cloudflare
        # rate-limiting that flags sessions reused across many requests
        # (Xiqu Centre/westk.hk Turnstile triggers on the 2nd+ request
        # from the same session). Each URL gets its own browser context
        # and 15s wait so Cloudflare's auto-resolve can finish.
        fresh_per_url = bool(venue_row.get("fresh_context_per_url", False))
        try:
        ...
        try:
            with sync_playwright() as p:
                launch_args = ["--disable-blink-features=AutomationControlled"] if use_stealth else []
                ...
                def _render(url: str) -> Optional[str]:
                    ...
                    if use_stealth and _Stealth is not None:
                        try:
                    ...
                    try:
                        ...
                        # If Cloudflare shows the "Performing security verification"
                        # interstitial, poll until it goes away (auto-resolves
                        # within ~5-30s for managed challenges).
                        try:
                            ...
                            pass
                        pg.wait_for_timeout(detail_wait)
                        html = pg.content()
                    except Exception as exc:
                        log.debug("  %s: detail render failed: %s", url, exc)
                        html = None
                    ctx.close()
                    return html

                if fresh_per_url or use_stealth:
                    for url in detail_urls:
                        html = _render(url)
                        if html:
                            detail_html_by_url[url] = html
                else:
                    ...
                    for url in detail_urls:
                        try:
                            page.goto(url, wait_until="domcontentloaded", timeout=timeout_ms)
                            page.wait_for_timeout(detail_wait)
                            detail_html_by_url[url] = page.content()
                        except Exception as exc:
                            log.debug("  %s: detail render failed: %s", url, exc)
                browser.close()
        except Exception as exc:
            log.warning("%s: detail-Playwright failed: %s", venue_row["id"], exc)

    sess = session or requests
    out: list[Event] = []
    for url in detail_urls:
        if use_pw_detail:
            html_str = detail_html_by_url.get(url)
            if not html_str:
                continue
...
def _tribe_to_event(raw: dict, venue_row: dict, venue_filter: str | None = None) -> Optional[Event]:
    ...
    # Optional filters mirrored from events-nrw' et4/toubiz parsers — useful
...
# Singapore district lookup for SISTIC venues. Patterns matched substring,
# lower-cased. Unmatched → "City Centre" (the largest, safest default).
_SG_VENUE_REGIONS = [
    # Sentosa & South
    (("sentosa", "resorts world", "universal studios"), "Sentosa & South"),
    # East (Kallang sports cluster, Tampines, Changi, Geylang, Katong)
    (("national stadium", "indoor stadium", "sports hub", "kallang",
      "tampines", "changi", "geylang", "katong", "paya lebar"), "East"),
    # West & North (Buona Vista, Jurong, Bukit Timah, Woodlands, NTU)
    (("the star", "*scape", "ntu", "nanyang technological", "jurong",
      "buona vista", "bukit timah", "woodlands", "yishun"), "West & North"),
    # Orchard & Central
    (("orchard", "tanglin", "dempsey", "holland village", "tiong bahru",
      "river valley"), "Orchard & Central"),
]


def _sistic_venue_region(venue_name: str) -> str:
    """Map a SISTIC venue_name string to one of the 5 SG district chips."""
    if not venue_name:
        return "City Centre"
    n = venue_name.lower()
    for needles, region in _SG_VENUE_REGIONS:
        if any(p in n for p in needles):
            return region
    return "City Centre"


def _scrape_sistic_api(venue_row: dict, session=None) -> list[Event]:
    """SISTIC paginated CMS API (cms.sistic.com.sg/.../api/events).

    The API caps per-page at 20 (limit>20 → 422). We page via `first=N` until
    `first >= total_records` or `max_pages` is hit. Each record:
      { id, title, alias, start_date "Wed, 30 Nov 2022",
        event_date "Wed, 30 Nov - Fri, 31 Dec 2027",
        venue_name, primary_genre, ... }
    `alias` maps to https://www.sistic.com.sg/events/<alias>.

    Config:
      calendar_url: base API URL (without first/limit)
      max_pages: safety cap (default 30 → up to 600 events)
      detail_url_template: format string with {alias}; default
    ...
    sep = "&" if "?" in base else "?"
    max_pages = int(venue_row.get("max_pages", 30))
    detail_tpl = venue_row.get("detail_url_template", "https://www.sistic.com.sg/events/{alias}")
    drop_genres = set(g.lower() for g in (venue_row.get("drop_genres") or []))
    headers = dict(DEFAULT_HEADERS)
    headers.update({"Accept": "application/json", "Referer": "https://www.sistic.com.sg/"})
    ...
    for page in range(max_pages):
        url = f"{base}{sep}first={first}&limit={limit}"
        try:
            r = sess.get(url, headers=headers, timeout=DEFAULT_TIMEOUT)
            r.raise_for_status()
            data = r.json()
        except (requests.RequestException, ValueError) as exc:
            log.warning("%s: sistic_api fetch failed first=%d: %s", venue_row["id"], first, exc)
            break
        records = data.get("data") or []
        if total is None:
            total = int(data.get("total_records") or 0)
            log.debug("%s: sistic total_records=%d", venue_row["id"], total)
        if not records:
            break
        for raw in records:
            title = _clean_title(_html_decode(raw.get("title") or ""))
            if not title:
                continue
            genre = (raw.get("primary_genre") or "").strip()
            if drop_genres and genre.lower() in drop_genres:
                continue
            # start_date is one specific day; event_date may be a range like
            # "Wed, 30 Nov - Fri, 31 Dec 2027". We use start_date for `start`
            # and try to parse the tail of event_date for `end`.
            def _strip_weekday(s):
                if not isinstance(s, str):
                    return ""
                s = s.strip()
                m = re.match(r"^[A-Za-z]{3,},\s*(.+)$", s)
                return m.group(1) if m else s
            sd = _strip_weekday(raw.get("start_date") or "")
            ed_raw = raw.get("event_date") or ""
            start = _parse_one(sd or _strip_weekday(ed_raw), date_prefer="future")
            if start is None:
                continue
            end = None
            if isinstance(ed_raw, str) and "-" in ed_raw:
                # Take text after the last "-"
                tail = _strip_weekday(ed_raw.rsplit("-", 1)[1].strip())
                end = _parse_one(tail, date_prefer="future")
            alias = raw.get("alias") or ""
            url_str = detail_tpl.format(alias=alias) if alias else venue_row.get("homepage", "#")
            if url_str in seen_urls:
                continue
            seen_urls.add(url_str)
        ...
        if total and first >= total:
            break
    log.info("%s: %d events from sistic_api (total advertised: %s)", venue_row["id"], len(out), total)
    return out
...
def _parse_tribe_dt(s, naive_is_utc: bool = False) -> Optional[datetime]:
    ...
    fields so we localize before converting to Asia/Singapore.
...
if __name__ == "__main__":
    ...
    p.add_argument("--venues-path", default="projects/events-nrw/config/venues.yaml")
