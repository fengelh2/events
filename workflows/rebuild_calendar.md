...
- `projects/events-nrw/config/venues.yaml`
- `projects/events-nrw/config/venue_aliases.json`
...
Cache raw responses in `projects/events-nrw/.tmp/raw/<venue_id>/<YYYY-MM-DD>.html` for debug. Do not use these as a fallback if the live fetch fails (stale data is worse than missing data here).
...
tools.render_events_html.render(
    events,
    out_path="projects/events-nrw/.tmp/events.html",
